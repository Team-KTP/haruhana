...
- 회원은 회원가입을 하고 로그인할 수 있다.
    ...
    - JWT 토큰 기반 인증이 사용된다.
- 회원 탈퇴 시 loginId와 nickname은 즉시 익명화된다. (`deleted_{id}_{uuid8}` 형식)
    - 익명화 목적: 탈퇴 후 동일한 ID/닉네임으로 재가입이 가능하도록 unique 제약 충돌 방지
...
- 회원은 가입 시 **반드시 오늘의 문제 설정을 완료**해야 한다.
    - 설정 항목은 다음과 같다.
      ...
      - 문제 카테고리 (소분류 토픽 1개)   -> 대분류 → 중분류 → 소분류(토픽)
- **Skip**은 불가능하다.
- 설정 직후 오늘의 문제가 제공된다.
  - 처음 설정일 경우에만 즉시 오늘의 문제가 제공된다.
- 설정은 **최대 5개**까지 추가 가능하다. (다중 카테고리 학습 지원)
  - 동일한 카테고리 토픽 + 난이도 조합의 중복 설정은 불가능하다.
- 특정 설정의 변경은 언제든 가능하나 변경 이후 이미 생성된 오늘의 문제에는 영향을 주지 않는다.
- 변경은 **다음날 자정(00:00)** 부터 적용된다.

#### 오늘의 문제 풀이
- 문제는 AI(Google Gemini)가 생성한다. 답변은 예시 형태로 제공된다. (V3 프롬프트 사용)
  - 생성 실패 시 백업 문제가 제공된다.
- **매일 자정(00:00)** 에 설정된 모든 난이도와 카테고리에 맞는 문제가 제공된다.
- 동일한 날짜 + 난이도 + 카테고리 조합에 대해 문제는 하나만 생성된다.
- 회원은 하루에 여러 문제를 받을 수 있다. (학습 설정 수만큼)
...
#### AI 채점
- 회원이 풀이를 제출(최초 제출 및 수정 제출 포함)하면 AI가 자동으로 채점한다.
- AI는 모범 답안과 회원의 답안을 비교하여 아래 항목을 평가한다.
  - 등급 (EXCELLENT / GOOD / FAIR / POOR)
  - 잘한 점
  - 부족한 점
  - 개선 방향
- 채점은 비동기로 처리되며, 제출 응답과 별개로 완료된다.
- 채점 결과는 제출 이력별로 독립적으로 저장된다. (수정 제출 시 이전 채점 결과 유지)
- 회원은 특정 제출에 대한 채점 이력 전체를 조회할 수 있다.
...
- 알림 종류 및 시점
  ...
  - 오전 학습 독려 알림 (DAILY_STUDY_REMINDER)
    - 매일 오전 08:30에 전체 회원에게 학습을 독려하는 알림이 발송된다.
...
- 탈퇴 시 `login_id`와 `nickname`은 `deleted_{id}_{uuid8자리}` 형식으로 즉시 익명화된다.

---

### MEMBER_PREFERENCE (회원 학습 설정)
...
#### 규칙
- 회원은 가입 직후 반드시 오늘의 문제 설정을 1개 이상 완료해야 한다.
- 설정은 **최대 5개**까지 추가할 수 있다.
- 동일한 `category_topic_id` + `difficulty` 조합의 중복 설정은 불허한다.
- 특정 설정의 변경은 언제든 가능하다. (preferenceId로 대상 지정)
- 변경은 다음날 자정(00:00)부터 적용된다.
- `effective_at`은 해당 설정이 적용되는 시작 날짜를 의미한다.
- 설정 변경 시 `effective_at`은 다음날 날짜로 저장된다.
- 같은 날 동일 설정 변경 시 기존 레코드를 업데이트한다(새 레코드 생성 없음).
...
#### 규칙
- 설정 직후 오늘의 문제가 제공된다. (캐시 즉시 무효화로 신규 문제 바로 반영)
- 회원은 학습 설정 수만큼 하루에 여러 문제를 가질 수 있다. (다중 카테고리 지원)
...
### SUBMISSION_FEEDBACK (AI 채점 결과)
- `submission_id` : 채점 대상 제출 ID (FK → SUBMISSION)
- `grade`         : 채점 등급 (EXCELLENT / GOOD / FAIR / POOR)
...
- 채점은 제출 완료 이벤트를 통해 비동기로 처리된다.
- 채점 등급:
  - `EXCELLENT` : 핵심 개념을 정확하고 깊이 있게 설명한 완성도 높은 답변
  - `GOOD`      : 핵심 개념은 정확하나 깊이 또는 구체성이 일부 부족
  - `FAIR`      : 개념은 일부 맞으나 설명이 피상적이거나 불명확
  - `POOR`      : 핵심 개념에 대한 이해가 부족하거나 질문 의도와 맞지 않음
...
- `type`      : 알림 유형 (DAILY_PROBLEM / STREAK_REMINDER / UNSOLVED_PROBLEM_REMINDER / DAILY_STUDY_REMINDER)
- `title`     : 알림 제목
- `body`      : 알림 내용
- `is_read`   : 읽음 여부
- `sent_at`   : 알림 발송 일시
...
- 알림 유형:
  ...
  - `DAILY_STUDY_REMINDER` : 매일 오전 08:30 전체 회원에게 학습 독려 알림 발송
